...
import { updateUserStart, updateUserSuccess, updateUserFailure, deleteUserFailure, deleteUserStart, deleteUserSuccess, singOutStart, signOutFailure, signOutSuccess } from '../redux/user/userSlice';
...
const Profile = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault(); //prevent reload button to erase the data
    try {
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <input id='username' onChange={handleChange} defaultValue={currentUser.username} type="text" placeholder='username' className='border p-3 rounded-lg' />
        <input id='email' onChange={handleChange} type="email" defaultValue={currentUser.email} placeholder='email' className='border p-3 rounded-lg' />
      ...
      {userListings && userListings.length > 0 &&
        <div className='flex flex-col gap-4'>
          <h1 className='text-center mt-7 text-2xl font-semibold'>Your Lisitngs</h1>
          {userListings.map((listing) => (
            <div key={listing._id}
              className='border rounded-lg p-3 flex justify-between items-center gap-4'>
              <Link to={`/lisitngs/${listing._id}`}>
